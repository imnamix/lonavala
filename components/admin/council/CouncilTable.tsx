...
export function CouncilTable() {
  ...
  const handleToggleActive = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await toggleCouncilMemberActive(id);
      setMembers((prev) =>
        prev.map((m) =>
  ...
  const handleDelete = async (id: string, name: string, e: React.MouseEvent) => {
    ...
    if (confirm(`Are you sure you want to remove ${name} from the Council directory?`)) {
      try {
        ...
        console.error("Failed to delete council member:", err);
        showToast("Failed to delete council member.");
      }
    }
  };
        ...
        <div className="overflow-x-auto rounded-2xl border border-border">
          {loading ? (
            <div className="p-12 text-center text-gray-500 space-y-3">
              ...
              <p className="text-xs font-semibold">Loading Council Members from API...</p>
            </div>
          ) : (
                        ...
                        <td className="py-3.5 px-4 text-center whitespace-nowrap">
                          ...
                          >
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-white border border-gray-200 hover:border-primary hover:bg-primary-light text-gray-600 hover:text-primary transition-colors cursor-pointer"
