...
import { ArrowRight, Quote, Award, CheckCircle2 } from "lucide-react";
...
export function CouncilOverview() {
  ...
  const chiefOfficer = COUNCIL_MEMBERS[2];

  const getOfficerDesignation = (id: string) => {
    if (id === "cm-2") return dict.council.vpDesignation;
    if (id === "cm-3") return dict.council.coDesignation;
    ...
    <section className="py-16 bg-white border-y border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gradient-to-br from-slate-50 via-white to-emerald-50/40 border border-slate-200 rounded-3xl p-6 sm:p-8 mb-6 shadow-xs hover:shadow-lg transition-all">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            <div className="md:col-span-4 lg:col-span-3 flex justify-center">
              <div className="relative w-36 h-44 sm:w-40 sm:h-48 rounded-2xl overflow-hidden shadow-md border-4 border-white ring-1 ring-slate-200">
                <Image
                  src={president.image}
                  alt={president.name}
                  fill
            ...
            <div className="md:col-span-8 lg:col-span-9 space-y-3">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900">
                {language === "mr" ? president.marathiName : president.name}
              </h3>
              ...
              <p className="text-xs sm:text-sm text-slate-600 italic border-l-2 border-emerald-600 pl-3 leading-relaxed">
                &ldquo;{dict.council.presidentQuote}&rdquo;
              </p>

              <div className="flex flex-wrap items-center gap-4 pt-1 text-xs text-slate-600">
                <span className="font-semibold text-slate-700">{dict.council.tenure}</span>
                <span>•</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {[vicePresident, chiefOfficer].map((officer) => (
            <div
              key={officer.id}
